...
fn collections(c: &mut Criterion) {
  ...
  c.bench_function("std::vec::Vec::push", |b| {
    b.iter_batched(
      || std::vec::Vec::<u8>::with_capacity(16),
  ...
  c.bench_function("std::vec::Vec::extend", |b| {
    b.iter_batched(
      || {
        (
          std::vec::Vec::<u8>::with_capacity(2048),
          core::iter::repeat(255).take(2048),
        )
      },
