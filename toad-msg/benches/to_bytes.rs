...
fn message_to_bytes(c: &mut Criterion) {
  ...
  for inp in inputs.iter() {
    ...
    group.bench_with_input(
      BenchmarkId::new("toad_msg/alloc/size", bytes.len()),
      inp,
      |b, inp| {
        b.iter_batched(
          || inp.get_alloc_message(),
    ...
    group.bench_with_input(
      BenchmarkId::new("toad_msg/no_alloc/size", bytes.len()),
      inp,
      |b, inp| {
        b.iter_batched(
          || inp.get_no_alloc_message::<4096, 32, 512>(),
    ...
    group.bench_with_input(
      BenchmarkId::new("coap_lite/size", bytes.len()),
      &cl_packet,
