...
mod route {
  ...
  pub fn done(
    ap: Ap<Hydrated, T<dtls::N>, (), io::Error>,
  ...
  pub fn time(
    ...
    ap.pipe(method::get)
      ...
      .bind_hydrated(|_, req| {
        ...
        let (duration_unit, duration_unit_fn) = query
          .into_iter()
          ...
          .and_then(|unit| match unit.as_str() {
            | "millis" => Some((
              unit,
              Box::new(|dur: Duration| dur.as_millis()) as Box<dyn Fn(Duration) -> u128>,
            )),
        ...
        respond::ok(
          format!(
            r#"{{ "you_are": {}, "unix_time": {}, "units": "{}" }}"#,
            req.addr(),
            duration_unit_fn(SystemTime::now().duration_since(UNIX_EPOCH).unwrap()),
            duration_unit
          )
          .into(),
        )
...
mod test {
  ...
  pub fn not_found(client: &P, addr: &str) {
    ...
    assert_eq!(
      resp.data().payload_string().unwrap(),
  ...
  pub fn hello(client: &P, addr: &str) {
    let (_, token) = client
      .send_msg(Addrd(
        Req::<T<dtls::N>>::get("hello/ethan").into(),
        addr.parse().unwrap(),
      ))
      .unwrap();
  ...
  pub fn observe(client: &P, addr: &str, unit: &str) {
    ...
    let (_, token) = client
      .send_msg(Addrd(register.into(), addr.parse().unwrap()))
      .unwrap();
    ...
    client
      .send_msg(Addrd(deregister.into(), addr.parse().unwrap()))
      .unwrap();
