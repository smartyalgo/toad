...
where
  T: Array<Item = (State<P::Clock>, Addrd<platform::Message<P>>)>,
  P: PlatformTypes,
{
}
  ...
  fn when_con_request_sent_retry_should_retry_appropriately() {
    ...
    s.inner()
      .set_poll_resp(|_, Snapshot { time, .. }, _, _, _| {
        let time: u64 = Milliseconds::try_from(time.duration_since_epoch())
          .unwrap()
          .0;
    ...
    macro_rules! sent {
      () => {{
        effs
          .iter()
          .filter(|e| matches!(e, Effect::Log(_, _)))
          .for_each(|e| match e {
