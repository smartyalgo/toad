...
mod tests {
  ...
  fn poll_req_emitting_single_register_request(
    ...
    static INVOCATIONS: Mutex<Option<HashMap<usize, usize>>> = Mutex::new(None);

    {
      let mut lock = INVOCATIONS.lock().unwrap();
      if lock.is_none() {
        *lock = Some(HashMap::default());
      }
    }

    move |_, _| {
      let mut inv_lock = INVOCATIONS.lock().unwrap();
      let mut map = inv_lock.take().unwrap();
